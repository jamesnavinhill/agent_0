...
export async function executeAutonomousTask(
  ...
  if (options.allowSubAgents !== false && delegations.length > 0) {
    ...
    const subAgentConfigs: SpawnSubAgentConfig[] = delegations.map(d => ({
      name: `Delegated ${(d.input as any).agentRole}`,
      role: (d.input as any).agentRole as SubAgentRole,
      task: (d.input as any).taskDescription,
    }))
